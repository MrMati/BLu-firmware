...
def bisect_right(lst, x, lo=0, hi=None):
    """Return the index where to insert item x in list lst, assuming lst is sorted.

    The return value i is such that all e in lst[:i] have e <= x, and all e in
    lst[i:] have e > x.  So if x already appears in the list, lst.insert(x) will
    insert just after the rightmost x already there.

    Optional args lo (default 0) and hi (default len(lst)) bound the
    slice of lst to be searched.
    """
...
TYPES = frozenset([
    TYPE_DOUBLE, TYPE_FLOAT, TYPE_INT, TYPE_UINT, TYPE_SINT, TYPE_FIXED32, TYPE_FIXED64, TYPE_SFIXED32, TYPE_SFIXED64,
    TYPE_BOOL, TYPE_STRING, TYPE_BYTES, TYPE_EMPTY,
    'v', 'V', 'l', 'L'
])
...
def _yield_fields_from_wire(buf, wire_type=None, field_number=None):
    ...
    while 1:
        ...
        if not wt_decoder:
            print(
                "ERROR: _yield_fields_from_wire():Ignore unknown type #%d", wire_type
            )
            continue
        try:
...
class _OverlapCheck:
    ...
    def _check_overlap(self, start, span=1):
        ...
        offset = bisect_right(parser_used_fields, start)
...
class Wire:
    ...
    def _encode_wire(self, stuff, fmtable=None):
        ...
        # Can be an index number or field name
        stuff_id = 0
        encoded = io.BytesIO()
        for fmt in fmtable:
            if self._kv_fmt or self._msg_cls:
    ...
    def _decode_field(self, field_type, field_data, subcontent=None):
        ...
        if field_type == TYPE_BYTES and subcontent:
            ...
            field_decoded = _decode_scalar_from_bytes(field_type, field_bytes, max_bits=self._vint_2sc_max_bits,
    ...
    def _decode_wire(self, buf, subfmt=None):
        ...
        for fmt in subfmt:
            ...
            # sanity check
            if self._kv_fmt or self._msg_cls:
                assert repeat == 1 or field_type == TYPE_EMPTY, \
            ...
            for field_id in range(field_id_start, field_id_start + repeat):
                ...
                # packed repeated field
                    ...
                    if len(fields) > 1:
                        print(
                            'ERROR: Multiple data found in a packed-repeated field.'
                        )
                ...
                # not a repeated field but has multiple data in one field
                    ...
                    print(
                        'ERROR: Multiple data found in a non-repeated field.'
                    )
...
class Field:
    ...
    def __init__(self, number, type_, required=False, repeated=False, repeated_packed=False):
        ...
        assert sum(
            [required, repeated, repeated_packed]) <= 1, "Can only speciffy 1 of required, repeated or repeated_Packed"
...
class Message:
    ...
    def __init__(self, **kwargs):
        ...
        assert name_to_fields_map is not None, \
            "Missing self.{}, forget to decorate Message with @process_message_fields?".format(
                _MESSAGE_NAME_TO_FIELDS_MAP)
